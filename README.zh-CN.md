...
**新版 Thunderbird 已原生支持 Microsoft Exchange（EWS）邮箱**：账户类型 `ews`
配合 Autodiscover（见 `email-exchange-type.mjs`、`ExchangeAutoDiscover.sys.mjs`、
`IExchangeOutgoingServer`、`ews_url`）。但该实现面向较新的 Exchange。

本项目所在环境是**已停服的 Exchange 2010 SP3**。其 EWS 在认证方式、schema 与
写入校验上都与新版存在差异，直接用原生 EWS 连接无法稳定工作
（例如服务端会以只读校验拒绝写操作，返回
`ErrorChangeKeyRequiredForWriteOperations`）。此外，当前 Thunderbird 的原生 EWS
**只覆盖邮箱**，并没有 EWS 日历/通讯录 provider（日历侧只有 CalDav/ICS 等）。

因此本项目在本机加一层**兼容 / 翻译桥**：

- **邮箱**：Thunderbird 原生 EWS 指向本地桥；桥完成 **NTLM 握手**并**重写请求**，
  以适配老服务器（并非协议翻译，仅做兼容补偿）。
