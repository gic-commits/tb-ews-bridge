...
- **转发时没有签名**：TB 默认关闭此项。账户设置 → 身份 → **撰写与地址** →
  勾选**「转发时包含签名」**（「回复包含签名」默认已勾选）。
- **回复/转发时签名落在历史邮件最下面**：同页把回复位置设为
  **「在引用内容之前回复」**，并把**「把签名档放在」**设为
  **「在我的回复之后（在引用内容之前）」**。
- **任何邮件都没有签名（含新邮件）**：账户设置主面板检查**「签名文字：」**
  已填写并点击确定保存；若勾选了**「附加文件……作为签名：」**但文件路径
  无效，文本签名也会被一并忽略——取消勾选或改好路径。
