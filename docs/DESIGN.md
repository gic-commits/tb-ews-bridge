...
│ Thunderbird │ ─────────────────── │ 17080 邮件桥  │ ──────────────────▶│ mail.example.com:│
│             │   CalDAV :17081      └──────────────┘   (curl --ntlm)    │ 443 Exchange 2010│
│             │ ───────────────────▶ ewscaldav.py ─┐                    └──────────────────┘
│             │   LDAP   :17089                     │ ews_bridge.py
...
> 端口默认 `17080`（邮件 EWS）/ `17081`（CalDAV）/ `17089`（LDAP），均可配置；
> 各服务配置位置与改端口时需要同步修改的地方见 README「端口及如何修改」。

## 2. `ews_bridge.py` — EWS 通道与邮件桥

职责：

1. **邮件透传（TCP 级）**：监听 17080，客户端连入后按请求头/体转发到上游，
